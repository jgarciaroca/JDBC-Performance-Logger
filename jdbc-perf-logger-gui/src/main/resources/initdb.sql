...
create table if not exists connection_info
    ...
    url varchar not null, creationDate timestamp not null, connectionCreationDurationNanos bigInt, connectionProperties other, clientId varchar default '');
...
create table if not exists statement_log
    ...
    threadName varchar, exception varchar, timeout int, autoCommit boolean, transaction_Isolation int, callerStackTrace varchar default '');
...
create or replace view v_statement_log
    (id, tstamp, statementType, rawSql, filledSql, EXEC_PLUS_RSET_USAGE_TIME, execution_time, fetch_time, RSET_USAGE_TIME, nbRows, threadName, timeout, autoCommit, transaction_Isolation, error, connectionNumber, clientId)
